...
1. (任意)参考にする**画像または動画**(競合の投稿・スクリーンショット等)をアップロードすると、
   ...
   動画を渡した場合はテロップが実際にどう動いて出てくるかも見て演出を判定する。
   判定は既定でProモデル(`gemini-pro-latest`)が行い、文字の太さ・縁取り・帯・差し色・配置を
   観察してから候補を選ぶ。スクリーンショットは細部を読めるよう高解像度で渡す。手動で
...
- クリップごとのAIナレーション追加要否(最大10クリップまで。入れすぎて元の話し声と
  ぶつからないようにするための歯止め)
- クリップごとの効果音プリセット配置
...
使われ提案の質が上がる。学習動画(編集前)も登録してあればペアで渡し、「素材に何を足したか」
の差分から編集者の癖を学ばせる。プランの生成は既定でProモデル(`gemini-pro-latest`)を使い、
フック・視聴維持・山場を設計させる。編集例の動画は低解像度で読ませてコストを抑えている。
...
- `src/lib/gemini/` — Gemini API呼び出しのラッパー
  ...
  - `autoEditPlan.ts` / `autoEditTypes.ts` / `autoEditJobs.ts` — 自動編集(バズる動画)の
    ...
    尺だけを渡す。`editExamplesStore.ts` に登録済みの編集例(学習動画+正解動画のペア、最大3組)が
    あれば、few-shotとして先頭に差し込む
